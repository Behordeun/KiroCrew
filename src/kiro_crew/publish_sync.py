...
async def unpublish(slug: str) -> None:
    ...
    retryable. The ONE exception is a typed ``DriveNotFound`` -- the destination is
    confirmed gone, so there is nothing left to withdraw and the record is released.
    Deleting the ARTIFACT is the other way out of a kept publication, and it does
    attempt the destination withdrawal first (see ``delete_for_artifact``) -- but it is
    NOT an escape hatch: a delete whose withdrawal did not happen is refused, so it
    cannot be used to walk away from a copy that is still served. THIS function is the
    deliberate way out: unpublishing is the user stating, in the open, that they accept
    the published copy may remain. Raises ``NotPublishedError`` only when the artifact
    isn't published.
    """
    store = get_default_store()
    art = await asyncio.to_thread(store.get, slug)
...
class DeleteWithdrawal(enum.Enum):
    ...
    refuses, loudly. Failing a delete is recoverable -- the user can retry, or use
    the explicit ``unpublish`` escape hatch and accept the consequence in the open --
    while a world-readable copy whose only handle we just erased is not.
