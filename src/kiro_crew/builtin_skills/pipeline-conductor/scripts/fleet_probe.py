...
def _program_class(cmd: str, fleet: list[str]) -> str:
    ...
    for root in fleet:
        if _under(literal, _norm_path(root)) or _under(real, _norm_path(os.path.realpath(root))):
            return "fleet"
...
def run_probe(cfg: dict[str, Any], state_path: Path) -> int:
    ...
    for key in sessions:
        ...
        if _suppressed(handled, key, tag, digest, idle_secs):
            # The report itself is dealt with. Whether anything has come OUT of
            # the session since is a different question, and the interesting case
            # for it is exactly here: the ruling was delivered, the tag went
            # quiet, and the worker then produced nothing at all. Without this the
            # named tag masks the stall for as long as it stays suppressed.
            if tag == NOPROGRESS_TAG or not _stalled_since_disposition(
                handled, key, index, idle_secs
            ):
                continue
            tag = NOPROGRESS_TAG
            digest = _digest(f"{tag}:{tail}")
...
def mark_handled(cfg: dict[str, Any], state_path: Path, key: str, tag: str, digest: str) -> int:
    ...
    if proto is not None:
        entry["proto"] = proto
    if tag in _EXPIRING_TAGS:
        # One entry per key, so this mark is about to overwrite whatever was
        # recorded before it. IDLE and NOPROGRESS describe a CONDITION; the thing
        # they overwrite may be a PAYLOAD that was answered, and losing that makes
        # an answered report re-present. Preserve it beside the new entry rather
        # than under it: the same shape ``proto`` already uses for the terminal
        # reading, generalised to the suppression itself.
        previous = handled.get(key)
        previous = previous if isinstance(previous, dict) else {}
        prior_tag, prior_digest = previous.get("tag"), previous.get("digest")
        if (
            isinstance(prior_tag, str)
            and prior_tag not in _EXPIRING_TAGS
            and isinstance(prior_digest, str)
        ):
            entry["settled"] = {"tag": prior_tag, "digest": prior_digest}
        elif isinstance(previous.get("settled"), dict):
            # Already condition-marked once; keep carrying the original payload.
