...
class WakaTimeConfig:
    ...
    api_base_url: str = field(
        ...
        metadata=_meta(
            ...
            "WakaTime API at https://wakatime.com/api/v1. Only honored when it "
            "is an https URL on wakatime.com, OR when Allow self-hosted backend "
            "is on; any other value falls back to the public API so the API key "
            "cannot be posted to an unapproved host.",
            tags=["wakatime"],
        ),
    )
    allow_self_hosted: bool = field(
        default=False,
        metadata=_meta(
            "Allow self-hosted backend",
            "Permit API Base URL to point at a self-hosted, API-compatible "
            "backend (Wakapi, Hackatime) instead of the public WakaTime API. "
            "Off by default: with it off, only an https wakatime.com base URL "
            "is honored and anything else falls back to the public API, so a "
            "config change alone cannot redirect the vault API key to an "
            "unapproved endpoint. Turn this on deliberately for a self-hosted "
            "backend you trust; the URL must still be https.",
            tags=["wakatime"],
        ),
    )
    send_heartbeats: bool = field(
        ...
        metadata=_meta(
            ...
            "the messaging channels that run through it (Slack and the other "
            "chat surfaces); the task runner, subagents, installed apps, and "
            "the standalone kirocrew chat CLI each run their own turn loop and "
            "do not emit heartbeats yet.",
            tags=["wakatime"],
        ),
    )
