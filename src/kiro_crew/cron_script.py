...
def _pin_payload(
    ...
    ``session_key`` is the job's delivery ownership — which session (and so
    which human's surface) receives the run's output. It is bound into the
    pin because the store is agent-writable: without it an agent could
    rewrite ``session_key`` under a still-valid pin and steer an approved
    script's granted output to another session's surface.
    """
...
def compute_secret_env_pin(
    ...
    *,
    ...
    another job), the SESSION KEY (delivery ownership — rewriting where the
    ...
    return _pin_digest(
        _pin_payload(script, command, message, body, job_id, grant, domain, session_key),
        domain=domain,
        job_id=job_id,
        epoch=epoch,
    )
...
def _secret_env_precheck(
    ...
    ``active`` domain with this job's id, session key, and mapping bound in
    ...
    try:
        current = _pin_digest(
            _pin_payload(
                script, command, message, script_body, job_id, secret_env, "active", session_key
            ),
            domain="active",
            job_id=job_id,
        )
