    ...
    def delete(
        ...
        * **True (cascade)** — permanently delete the whole subtree: every
        ...
        with self._lock:
            ...
            # block, so a failed delete leaves the folder present with its
            # guard still armed rather than resetting it to 0 and letting a
            # stale in-flight generation clobber a manual icon. After a
            # confirmed delete the entries have nothing left to guard
            # (set_icon_if_epoch already drops a folder it cannot re-find);
            # popping keeps the registry from growing with every deleted id.
